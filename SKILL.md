...
- **Shopee site enum (verified)**: `MY`, `PH`, `VN`, `TH`, `ID`, `SG`, `TW`, `BR` (plus `UnKonw` default). Always pass `site` explicitly — it's optional in schema but server behavior without it is inconsistent.
